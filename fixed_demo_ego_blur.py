...
def visualize_video(
    ...
    # Define video writer with OpenCV
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for MP4 format
    video_writer = cv2.VideoWriter(output_video_path, fourcc, output_video_fps, (width, height))

    total_frames = int(video_reader_clip.duration * video_reader_clip.fps)
    progress_bar = tqdm(total=total_frames, desc="Processing frames")

    for i, frame in enumerate(video_reader_clip.iter_frames()):
        # Convert grayscale to RGB if needed and copy frame
        ...
        visualized_image = visualize(image, detections, scale_factor_detections)

        # Convert to BGR for OpenCV and write to video
        visualized_image_bgr = cv2.cvtColor(visualized_image, cv2.COLOR_RGB2BGR)
        video_writer.write(visualized_image_bgr)

        # Explicitly free memory after processing each frame
        del image, bgr_image, image_tensor, detections, visualized_image, visualized_image_bgr

            
        # Update progress every 10%
        if i % (total_frames // 10) == 0:
            progress_bar.update(total_frames // 10)
