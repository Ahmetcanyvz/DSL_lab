...
def save_timestamp_to_mp4(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # Write metadata to a temporary file with timestamps in a manageable format
        _description_metadata_filepath = os.path.join(temp_dir, "description-metadata.txt")
        with open(_description_metadata_filepath, "w") as f:
            f.write(";FFMETADATA1\n")
            # Write each timestamp in a way that minimizes argument length issues
            f.write(f"description={json.dumps(timestamp_array)}")  # Using JSON format to save space

        # Run the ffmpeg command to add metadata from the file
        cmd = [
            ffmpeg_binary,
            "-i",
            input_video_file,
